...
function renderAll(data){const d=data||fallback,settings=d.settings||fallback.settings,services=Array.isArray(d.services)?d.services:fallback.services,projects=Array.isArray(d.projects)?d.projects:fallback.projects,statistics=Array.isArray(d.statistics)?d.statistics:fallback.statistics,technologies=Array.isArray(d.technologies)?d.technologies:fallback.technologies,skills=Array.isArray(d.skills)?d.skills:[],testimonials=Array.isArray(d.testimonials)?d.testimonials:fallback.testimonials,experiences=Array.isArray(d.experiences)?d.experiences:[],socials=Array.isArray(d.social_links)?d.social_links:[];renderSettings(settings,socials);renderServices(services);renderProjects(projects);renderStats(statistics);renderTech(technologies);renderSkills(skills);renderTestimonials(testimonials);renderExperiences(experiences);renderDetail(projects);renderFilters(projects);applySeo(settings);document.documentElement.classList.add('content-ready')}
...
function renderSkills(items){const el=$('#skillList');if(!el)return;const list=Array.isArray(items)?items:[];el.innerHTML=list.map(x=>`<div class="skill"><div class="skill-head"><b>${esc(x.name)}</b><span>${Number(x.level||0)}%</span></div><div class="bar"><span style="width:${Math.min(100,Number(x.level||0))}%"></span></div></div>`).join('');el.hidden=list.length===0}
