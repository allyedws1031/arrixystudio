...
async function projects(){
 ...
 const p=editing||{};
 $('#adminContent').innerHTML=`<h2>Projetos</h2><p class="subtitle">Cadastre projetos com imagens e vídeos enviados diretamente do computador ou por link.</p>
 <form id="projectForm">
  <div class="form-grid three">${field(['title','Título'],p.title)}${field(['slug','Slug'],p.slug)}${field(['category','Categoria'],p.category)}${field(['project_url','Link do projeto'],p.project_url)}${field(['order_index','Ordem','number'],p.order_index||0)}${field(['media_type','Mídia principal'],p.media_type||'image')}</div>
  ${field(['description','Descrição','textarea'],p.description)}
  <section class="admin-section"><h3>Imagem de capa</h3><div class="form-grid">${field(['cover','URL da capa (opcional)'],p.cover)}${uploadField('coverFile','Enviar capa do computador','image/*',p.cover)}</div></section>
  <section class="admin-section"><h3>Banner</h3><div class="form-grid">${field(['banner','URL do banner (opcional)'],p.banner)}${uploadField('bannerFile','Enviar banner do computador','image/*',p.banner)}</div></section>
  <section class="admin-section"><h3>Vídeo do projeto</h3><div class="form-grid">${field(['video_url','URL de vídeo ou embed (YouTube/Vimeo)'],p.video_url)}${uploadField('videoFile','Enviar MP4, WebM ou OGG','video/mp4,video/webm,video/ogg',p.video_url)}</div><p class="subtitle">Ao enviar um arquivo, ele será salvo no bucket portfolio-media. Links do YouTube e Vimeo serão exibidos em iframe.</p></section>
  <div class="form-grid">${field(['problem','Problema','textarea'],p.problem)}${field(['solution','Solução','textarea'],p.solution)}${field(['results','Resultados','textarea'],p.results)}</div>
  <div class="field"><label>Tecnologias (separadas por vírgula)</label><input name="technologies" value="${esc((p.technologies||[]).join(', '))}"></div>
  <section class="admin-section"><h3>Galeria</h3><div class="field"><label>URLs atuais (uma por linha)</label><textarea name="gallery">${esc((p.gallery||[]).join('\n'))}</textarea></div><div class="field"><label>Adicionar imagens do computador</label><input id="galleryFiles" type="file" accept="image/*" multiple><small>Você pode selecionar várias imagens de uma vez.</small></div></section>
  <div class="form-grid">${field(['featured','Projeto em destaque','checkbox'],p.featured)}${field(['active','Projeto visível','checkbox'],p.active)}</div>
  <div id="uploadStatus" class="form-message"></div><button class="btn pink" type="submit">${editing?'Atualizar':'Adicionar'} projeto</button>${editing?'<button class="btn light" type="button" id="cancelEdit">Cancelar</button>':''}
 </form><div class="admin-list">${data.map(x=>item(x,x.title,x.category,x.cover||x.banner)).join('')}</div>`;
 $('#projectForm').onsubmit=async e=>{
  e.preventDefault(); const form=e.target,btn=form.querySelector('button[type=submit]'),status=$('#uploadStatus'); btn.disabled=true; status.textContent='Enviando arquivos e salvando...';
  try{
   ...
   if(coverFile)payload.cover=await uploadMedia(coverFile,'projects/covers');
   if(bannerFile)payload.banner=await uploadMedia(bannerFile,'projects/banners');
   if(videoFile){payload.video_url=await uploadMedia(videoFile,'projects/videos');payload.video_type='upload';payload.media_type='video'}
   else if(payload.video_url){payload.video_url=embedUrl(payload.video_url);payload.video_type=/youtube\.com\/embed|player\.vimeo\.com/.test(payload.video_url)?'embed':'upload';}
   const currentGallery=(payload.gallery||'').split('\n').map(x=>x.trim()).filter(Boolean); for(const f of galleryFiles)currentGallery.push(await uploadMedia(f,'projects/gallery'));
   ...
   const {error}=await supabase.from('projects').upsert(payload,{onConflict:'slug'}); if(error)throw error;
   toast('Projeto salvo com sucesso'); editing=null; projects();
  }catch(err){toast(err.message||'Erro ao salvar',true);status.textContent=err.message||'Erro ao salvar'}finally{btn.disabled=false}
 };
 $('#cancelEdit')?.addEventListener('click',()=>{editing=null;projects()}); bindList('projects',data,projects)
}
