...
function isEmbedVideo(url=''){return /youtube\.com\/embed|player\.vimeo\.com\/video/.test(String(url))}
function projectMedia(p,context='card'){
 const video=p.video_url;
 if(video){
  if(isEmbedVideo(video))return `<iframe class="project-media ${context}" src="${video}" title="Vídeo de ${p.title||'projeto'}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
  return `<video class="project-media ${context}" src="${video}" controls ${context==='card'?'muted playsinline':''} preload="metadata"></video>`;
 }
 const image=p.banner||p.cover;
 return image?`<img class="project-media ${context}" src="${image}" alt="${p.title||'Projeto'}" loading="lazy" decoding="async">`:'';
}
...
function card(p,i){const media=(p.media_type==='video'&&p.video_url)?projectMedia(p,'card'):(p.cover?`<img src="${p.cover}" alt="${p.title||'Projeto'}" loading="lazy" decoding="async">`:projectMedia(p,'card'));return `<a class="project-card reveal" href="projeto.html?slug=${p.slug||p.id}"><div class="project-cover ${coverClass(p,i)}">${media}<h3>${p.title}</h3>${media?'':coverDecor(p)}</div><div class="project-content"><small>${p.category||'Projeto'}</small><p>${p.description||''}</p><div class="tags">${(p.technologies||[]).slice(0,3).map(t=>`<span class="tag">${t}</span>`).join('')}</div></div></a>`}
...
function renderProjectDetail(items){ const el=$('#projectDetail'); if(!el)return; const p=items.find(x=>(x.slug||String(x.id))===slug)||items[0]; document.title=`${p.title} — Artrixy Studio`; const gallery=(p.gallery||[]).map(url=>`<img src="${url}" alt="Galeria de ${p.title}" loading="lazy" decoding="async">`).join(''); el.innerHTML=`<section class="section project-detail-hero"><div class="project-banner">${projectMedia(p,'detail')}<div class="project-banner-content"><small>${p.category||''}</small><h1>${p.title}</h1><p>${p.description||''}</p></div></div></section><section class="section detail-grid"><div class="detail-box"><small>PROBLEMA</small><p>${p.problem||'Texto editável no painel.'}</p></div><div class="detail-box"><small>SOLUÇÃO</small><p>${p.solution||'Texto editável no painel.'}</p></div><div class="detail-box"><small>RESULTADOS</small><p>${p.results||'Texto editável no painel.'}</p></div><div class="detail-box"><small>TECNOLOGIAS</small><div class="tags">${(p.technologies||[]).map(t=>`<span class="tag">${t}</span>`).join('')}</div></div></section><section class="section"><small>GALERIA / MOCKUPS</small><div class="gallery">${gallery||'<p>Nenhuma imagem adicionada.</p>'}</div>${p.project_url?`<p><a class="btn pink" href="${p.project_url}" target="_blank" rel="noopener">Visitar projeto ↗</a></p>`:''}</section>`; }
